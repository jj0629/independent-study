...
D3D12_HANDLE_BUNDLE DX12Helper::GetNextCBVSRVHeapLocationHandles()
{
	D3D12_CPU_DESCRIPTOR_HANDLE cpuHandleStart = cbvSrvDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
	D3D12_GPU_DESCRIPTOR_HANDLE gpuHandleStart = cbvSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart();

	cpuHandleStart.ptr += (SIZE_T)srvDescriptorOffset * cbvSrvDescriptorHeapIncrementSize;
	gpuHandleStart.ptr += (SIZE_T)srvDescriptorOffset * cbvSrvDescriptorHeapIncrementSize;

	srvDescriptorOffset += 1;

	D3D12_HANDLE_BUNDLE payload = {};
	payload.cpuHandle = cpuHandleStart;
	payload.gpuHandle = gpuHandleStart;
...
D3D12_CPU_DESCRIPTOR_HANDLE DX12Helper::LoadCubeMap(const wchar_t* file, bool generateMips)
{
	...
	bool isCubeMap = true;
	CreateDDSTextureFromFile(device.Get(), upload, file, cubeMap.GetAddressOf(), generateMips, 0Ui64, 0, &isCubeMap);
	...
	desc.Format = cubeMap.Get()->GetDesc().Format;
	desc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURECUBE;
	desc.TextureCube.MipLevels = cubeMap.Get()->GetDesc().MipLevels;
	...
	device->CreateShaderResourceView(cubeMap.Get(), &desc, cpuHandle);
...
D3D12_CPU_DESCRIPTOR_HANDLE DX12Helper::LoadTexture(Microsoft::WRL::ComPtr<ID3D12Resource> texture)
{
	// Now that we have the texture, add to our list and make a CPU-side descriptor heap
	// just for this texture's SRV.  Note that it would probably be better to put all 
	// texture SRVs into the same descriptor heap, but we don't know how many we'll need
...
Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> DX12Helper::GetRTVHeap()
{
	return rtvHeap;
}

/// <summary>
/// Creates an RTV and its necessary components
/// </summary>
/// <param name="texDesc">A descripttion of the ID3D12Resource to create</param>
/// <param name="rtvDesc">A description of the render target view to create</param>
/// <param name="isScreenSize">Lets us know to flag this RTV appropriately</param>
/// <returns></returns>
RtvSrvBundle DX12Helper::CreateRtvSrvBundle(D3D12_RESOURCE_DESC texDesc, D3D12_RENDER_TARGET_VIEW_DESC rtvDesc, bool isScreenSize)
{
	Microsoft::WRL::ComPtr<ID3D12Resource> texture;
	...
	device->CreateCommittedResource(&heapDesc, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_ALLOW_ALL_BUFFERS_AND_TEXTURES, &texDesc, D3D12_RESOURCE_STATE_COMMON | D3D12_RESOURCE_STATE_RENDER_TARGET, 0, IID_PPV_ARGS(texture.GetAddressOf()));

	// Grab the actual heap start on both sides and offset to the next open SRV portion
	D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle = rtvHeap->GetCPUDescriptorHandleForHeapStart();
	D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle = rtvHeap->GetGPUDescriptorHandleForHeapStart();

	cpuHandle.ptr += (SIZE_T)rtvDescriptorOffset * rtvDescriptorSize;
	gpuHandle.ptr += (SIZE_T)rtvDescriptorOffset * rtvDescriptorSize;

	device->CreateRenderTargetView(texture.Get(), &rtvDesc, cpuHandle);

	rtvDescriptorOffset++;

	RtvSrvBundle payload;
	payload.rtvHandle = cpuHandle;
	payload.srvCPUHandle = LoadTexture(texture);
	payload.srvGPUHandle = this->CopySRVsToDescriptorHeapAndGetGPUDescriptorHandle(payload.srvCPUHandle, 1);
	payload.texResource = texture;
	payload.isScreenSized = isScreenSize;

	return payload;
}

/// <summary>
/// Creates an RTV with an existing texture (For use if multiiple RTVs share a texture)
/// </summary>
/// <param name="texture">An already-created texture resource</param>
/// <param name="rtvDesc">A description of the render target view to create</param>
/// <param name="isScreenSize">Lets us know to flag this RTV appropriately</param>
/// <returns></returns>
RtvSrvBundle DX12Helper::CreateRtvSrvBundle(Microsoft::WRL::ComPtr<ID3D12Resource> texture, D3D12_RENDER_TARGET_VIEW_DESC rtvDesc, bool isScreenSize)
{
	// Grab the actual heap start on both sides and offset to the next open SRV portion
	D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle = rtvHeap->GetCPUDescriptorHandleForHeapStart();
	D3D12_GPU_DESCRIPTOR_HANDLE gpuHandle = rtvHeap->GetGPUDescriptorHandleForHeapStart();
	...
	Microsoft::WRL::ComPtr<IDXGISwapChain> swapChain)
{
	...
	this->CreateConstantBufferUploadHeap();
	this->CreateCBVSRVDescriptorHeap();
	this->CreateRTVHeap();
}

void DX12Helper::SetWidth(unsigned int w)
{
	this->width = w;
}

void DX12Helper::SetHeight(unsigned int h)
{
	this->height = h;
}

unsigned int DX12Helper::GetWidth()
{
	return this->width;
}

unsigned int DX12Helper::GetHeight()
{
	return this->height;
}
...
void DX12Helper::CreateConstantBufferUploadHeap()
{
	...
	cbUploadHeapSizeInBytes = maxConstantBuffers * (UINT64)256;
