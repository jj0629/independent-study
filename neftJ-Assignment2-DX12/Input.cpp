...
bool Input::KeyPress(int key)
{
	...
	return
		...
		!(prevKbState[key] & 0x80) && !guiWantsKeyboard;		// Up last frame
}
...
bool Input::KeyRelease(int key)
{
	...
	return
		...
		prevKbState[key] & 0x80 && !guiWantsKeyboard;	// Down last frame
}
...
void Input::SetGuiKeyboardCapture(bool capture)
{
	guiWantsKeyboard = capture;
}

void Input::SetGuiMouseCapture(bool capture)
{
